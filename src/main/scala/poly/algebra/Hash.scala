 ...
 */
trait Hash[@miniboxed X] extends Eq[X] {
...
object Hash {
  ...
  def create[@miniboxed X](fHash: X => Int)(implicit E: Eq[X]): Hash[X] = new Hash[X] {
    def eq(x: X, y: X) = E.eq(x, y)
    override def ne(x: X, y: X) = E.ne(x, y)
    def hash(x: X): Int = fHash(x)
  }

  implicit def default[@miniboxed X]: Hash[X] = new Hash[X] {
    def eq(x: X, y: X) = x == y
