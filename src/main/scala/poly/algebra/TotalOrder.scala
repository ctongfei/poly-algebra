 ...
 */
trait TotalOrder[@sp(di) X] extends Lattice[X] with WeakOrder[X] { self =>
...
object TotalOrder {
  ...
  def apply[@sp(di) X](implicit O: TotalOrder[X]) = O

  def create[@sp(di) X](fLt: (X, X) => Boolean) = new TotalOrder[X] {
    def cmp(x: X, y: X) = if (fLt(x, y)) -1 else if (x == y) 0 else 1
    override def lt(x: X, y: X) = fLt(x, y)
    override def gt(x: X, y: X) = fLt(y, x)
  }

  def by[Y, @sp(di) X](f: Y => X)(implicit O: TotalOrder[X]) = new TotalOrder[Y] {
