 ...
 */
trait InnerProductSpace[V, @specialized(Double) F] extends NormedVectorSpace[V, F] {
...
object InnerProductSpace {
  def apply[V, @specialized(Double) F](implicit S: InnerProductSpace[V, F]) = S
  def create[V, @specialized(Double) F: PowerOps : Field](fDot: (V, V) => F)(implicit S: VectorSpace[V, F]) = new InnerProductSpace[V, F] {
